...
void delay_ms(uint32_t ms) {  /// delay in ms (1,2^16);
                              /// TIM4 runs on APB1 @ 42MHz with multiplier 2
                              /// which makes it 84MHz since it takes some time
                              /// to setup timer the actual delay will be ms +
                              /// const! function error < 0.3288%
...
void delay_us(
    uint32_t us) {  /// delay in us (1,2^16);
                    /// TIM4 runs on APB1 @ 42MHz with multiplier 2 which makes
                    /// it 84MHz since it takes some time to setup timer the
                    /// actual delay will be us + const! error < 0.1004%
