...
while ((HAL_GPIO_ReadPin(GPIOA,GPIO_PIN_1)));  //ceka se 0 na pinu 
...
uint8_t i,j;
for(j = 0; j < 8; j++){
while(!(HAL_GPIO_ReadPin(GPIOA,GPIO_PIN_1)));   // cekamo visoki nivo, tj. log 1
...
i&=~(1<<(7-j)); // upis 0 ;
	...
	i|=(1<<(7-j));  // upisemo 1
while((HAL_GPIO_ReadPin(GPIOA,GPIO_PIN_1)));   //cekamo 0 
}
return i;
}
