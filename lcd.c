 ...
 * and meaningful to the library itself(not the library user) uses this
 * variable.
 */
...
static void write4Bits(uint8_t val) {
  HAL_GPIO_WritePin(__lcd->D7->GPIOx, __lcd->D7->GPIO_Pin_x,
                    SET_IF(val & 0x08));  // 4th bit of lower nibble
  HAL_GPIO_WritePin(__lcd->D6->GPIOx, __lcd->D6->GPIO_Pin_x,
                    SET_IF(val & 0x04));
  HAL_GPIO_WritePin(__lcd->D5->GPIOx, __lcd->D5->GPIO_Pin_x,
                    SET_IF(val & 0x02));
  HAL_GPIO_WritePin(__lcd->D4->GPIOx, __lcd->D4->GPIO_Pin_x,
                    SET_IF(val & 0x01));  // 0th bit(LSb)
  enable();
}
...
static void write8Bits(uint8_t val) {
  HAL_GPIO_WritePin(__lcd->D0->GPIOx, __lcd->D0->GPIO_Pin_x,
                    SET_IF(val & 0x80));  // 8th bit(starting from 0-LSb)
  HAL_GPIO_WritePin(__lcd->D1->GPIOx, __lcd->D1->GPIO_Pin_x,
                    SET_IF(val & 0x40));
  HAL_GPIO_WritePin(__lcd->D2->GPIOx, __lcd->D2->GPIO_Pin_x,
                    SET_IF(val & 0x20));
  HAL_GPIO_WritePin(__lcd->D3->GPIOx, __lcd->D3->GPIO_Pin_x,
                    SET_IF(val & 0x10));
  HAL_GPIO_WritePin(__lcd->D4->GPIOx, __lcd->D4->GPIO_Pin_x,
                    SET_IF(val & 0x08));  // 4th bit
  HAL_GPIO_WritePin(__lcd->D5->GPIOx, __lcd->D5->GPIO_Pin_x,
                    SET_IF(val & 0x04));
  HAL_GPIO_WritePin(__lcd->D6->GPIOx, __lcd->D6->GPIO_Pin_x,
                    SET_IF(val & 0x02));
...
void sendCommand(uint8_t cmd) {
  HAL_GPIO_WritePin(__lcd->RS->GPIOx, __lcd->RS->GPIO_Pin_x,
                    GPIO_PIN_RESET);  // Command mode
  HAL_GPIO_WritePin(__lcd->RW->GPIOx, __lcd->RW->GPIO_Pin_x,
                    GPIO_PIN_RESET);  // Write mode
...
#ifdef EIGHT_BIT_MODE
                      PortPin_Map *D0, PortPin_Map *D1, PortPin_Map *D2,
                      PortPin_Map *D3,
#endif
                      PortPin_Map *D4, PortPin_Map *D5, PortPin_Map *D6,
                      PortPin_Map *D7) {
  LCD_TypeDef *ret;
